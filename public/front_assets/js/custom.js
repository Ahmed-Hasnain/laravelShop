/** 
  ...
  * Author URI: http://www.markups.io/
  ...
  1. CARTBOX 2. TOOLTIP 3. PRODUCT VIEW SLIDER 4. POPULAR PRODUCT SLIDER
  (SLICK SLIDER) 5. FEATURED PRODUCT SLIDER (SLICK SLIDER) 6. LATEST PRODUCT
  SLIDER (SLICK SLIDER) 7. TESTIMONIAL SLIDER (SLICK SLIDER) 8. CLIENT BRAND
  SLIDER (SLICK SLIDER) 9. PRICE SLIDER  (noUiSlider SLIDER) 10. SCROLL TOP
  BUTTON 11. PRELOADER 12. GRID AND LIST LAYOUT CHANGER 13. RELATED ITEM
  SLIDER (SLICK SLIDER)
...
function home_add_to_cart(id,size_id,color_id){
  ...
  add_to_cart(id,size_id,color_id);
}

function add_to_cart(id,size_id,color_id){
  ...
  {
    jQuery('#add_to_cart_msg').html('<div class="alert alert-primary" role="alert" id="add_to_cart_msg">Please Select Size</div>');
  }
  else if(color_id == '' && color_id != 'no')
  {
    jQuery('#add_to_cart_msg').html('<div class="alert alert-primary" role="alert" id="add_to_cart_msg">Please Select Color</div>');
  }
  else
  {
    ...
    jQuery.ajax({
      ...
      success:function(result){
        ...
        if(result.totalCartItem == 0){
          ...
          jQuery('.aa-cart-notify').html(result.totalCartItem);
          var html = '<ul>';
          jQuery.each(result.data,function(arrKey,arrVal){
            totalPrice = parseInt(totalPrice) + (parseInt(arrVal.qty) * parseInt(arrVal.price));
              html+= '<li><a class="aa-cartbox-img" href="'+product_slug+'/'+arrVal.slug+'"><img src="'+product_image+'/'+arrVal.image+'" alt="img"></a><div class="aa-cartbox-info"><h4><a href="'+product_slug+'/'+arrVal.slug+'">'+arrVal.name+'</a></h4><p>'+arrVal.qty+' x Rs '+arrVal.price+'</p></div><a class="aa-remove-product" href="#"><span class="fa fa-times"></span></a></li>'
        ...
        html+='</ul>';
        html+='<a class="aa-cartbox-checkout aa-primary-btn" href="'+product_slug+'/checkout">Checkout</a>'
        jQuery('.aa-cartbox-summary').html(html);
      }
    });
    // jQuery('#add_to_cart_msg').html('<div class="alert alert-primary" role="alert" id="add_to_cart_msg">Thanks</div>');
  }
}

function deleteCartProduct(pid,color,size,attr_id){
  jQuery('#color_id').val(color);
  jQuery('#size_id').val(size);
...
function color_filter(color,type){
  var color_str = jQuery('#color-filter').val();
  if(type == 1){
    var new_color = color_str.replace(color+':','')
    jQuery('#color-filter').val(new_color);
  }
  else{
    jQuery('#color-filter').val(color+':'+color_str);
  }
...
function funSearch(){
  var searched_str = jQuery('#search_str').val();
  if(searched_str != '' && searched_str.length > 2)
  {
      window.location.href='/search/'+searched_str;
  }
}
  ...
  jQuery.ajax({
    ...
    success: function(result){
      ...
      if (result.status == "success") {
          ...
          jQuery('#register_success_msg').html(result.msg);
        }
...
jQuery('#frmUpdatePassword').submit(function(e){
  e.preventDefault();
   jQuery('#register_success_msg').html('');
  jQuery.ajax({
    ...
    jQuery.ajax({
      ...
      success:function(result){
        ...
        if (result.status == 'success') {
          ...
          jQuery('#coupon_code_value').html('RS '+result.discounted_amount);
          jQuery('#total_price').html('RS '+result.cartTotalAmount);
...
jQuery('#frmProductReview').submit(function(e){
  ...
  jQuery.ajax({
    ...
    success: function(result){      
     if (result.status == 'success') 
     {
        jQuery('#product_review_msg').html(result.msg); 
